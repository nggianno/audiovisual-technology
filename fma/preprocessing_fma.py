...
def preprocess_csv_files(tracks, echonest, features):
    # give track_id columns the same name for merging columns afterwards
...
def make_tracks_dataset(data, features):
    ...
    genre1 = dataset[dataset['genre_top'] == 'Classical'].head(max_tracks_each_genre)
    genre2 = dataset[dataset['genre_top'] == 'Rock'].head(max_tracks_each_genre)
    genre3 = dataset[dataset['genre_top'] == 'Hip-Hop'].head(max_tracks_each_genre)
    genre4 = dataset[dataset['genre_top'] == 'Folk'].head(max_tracks_each_genre)
    # append each to our final dataset
    final_dataset = pd.DataFrame()
    final_dataset = final_dataset.append([genre1, genre2, genre3, genre4])

    return final_dataset


def extract_to_csv(df):
    df.to_csv(path_or_buf='/home/nick/Desktop/yliko_sxolhs/AudioVisual Technology/fma_metadata/final8.csv')
...
if __name__ == '__main__':
    ...
    final = make_tracks_dataset(tracks, features)
